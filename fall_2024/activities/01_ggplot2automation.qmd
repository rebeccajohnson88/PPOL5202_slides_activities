...
mean_values <- midwest %>%
  filter(state =='IL' & !county %in% focal_county) %>%
  summarize(
    perchsd_mean = mean(perchsd, na.rm = TRUE),
    percollege_mean = mean(percollege, na.rm = TRUE),
    percprof_mean = mean(percprof, na.rm = TRUE)
  )
